...
fn find_highlight(old_line: &str, new_line: &str) -> (Option<(usize, usize)>, Option<(usize, usize)>) {
    ...
    while old_suffix < old_chars.len() - prefix && new_suffix < new_chars.len() - prefix && old_chars[old_chars.len() - 1 - old_suffix] == new_chars[new_chars.len() - 1 - new_suffix] {
        old_suffix += 1;
        new_suffix += 1;
    }
    ...
    let old_hl = if prefix < old_end { Some((prefix, old_end)) } else { None };
    let new_hl = if prefix < new_end { Some((prefix, new_end)) } else { None };
...
pub fn render_diff_text(ui: &mut egui::Ui, diff_text: &str) {
    ...
    while i < parsed_lines.len() {
        if parsed_lines[i].line_type == LineType::Deleted && i + 1 < parsed_lines.len() && parsed_lines[i+1].line_type == LineType::Added {
            let old_text = parsed_lines[i].text.strip_prefix('-').unwrap_or(parsed_lines[i].text);
            let new_text = parsed_lines[i+1].text.strip_prefix('+').unwrap_or(parsed_lines[i+1].text);
    ...
    for p_line in &parsed_lines {
        ...
        let old_num = p_line.old_line_num.map(|n| n.to_string()).unwrap_or_default();
        let new_num = p_line.new_line_num.map(|n| n.to_string()).unwrap_or_default();
        
        let selectable = p_line.line_type == LineType::Added || p_line.line_type == LineType::Deleted;
        ...
        egui::Frame::default()
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    let row_height = 20.0;
                    let (gutter_rect, gutter_resp) = ui
                        .allocate_exact_size(Vec2::new(gutter_width, row_height), if selectable { egui::Sense::click_and_drag() } else { egui::Sense::hover() });
                    ...
                    let (handle_rect, handle_resp) = ui.allocate_exact_size(Vec2::new(handle_width, row_height), if selectable { egui::Sense::click() } else { egui::Sense::hover() });
                    
                    if handle_resp.hovered() && selectable {
                        ui.output_mut(|o| o.cursor_icon = egui::CursorIcon::PointingHand);
                        ui.painter().rect_filled(handle_rect, 0.0, Color32::from_white_alpha(20));
                    }
                    ui.painter().vline(
                        handle_rect.right(),
                        handle_rect.y_range(),
                    ...
                    ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| {
                        if let Some((hl_start, hl_end)) = p_line.char_highlight {
                            let chars: Vec<char> = p_line.text.chars().collect();
                            let prefix: String = chars.iter().take(hl_start).collect();
                            let highlight: String = chars.iter().skip(hl_start).take(hl_end - hl_start).collect();
                            let suffix: String = chars.iter().skip(hl_end).collect();
                            ...
                            if !prefix.is_empty() { job.append(&prefix, 0.0, fmt.clone()); }
                            
                            let mut hl_fmt = fmt.clone();
                            hl_fmt.background = hl_bg.unwrap_or(Color32::TRANSPARENT);
                            
                            if !highlight.is_empty() { job.append(&highlight, 0.0, hl_fmt); }
                            
                            if !suffix.is_empty() { job.append(&suffix, 0.0, fmt); }
